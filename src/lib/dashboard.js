...
export function computeExecutiveMetrics({ projects, actions }) {
  ...
  // "Projects requiring decision" = projects with a pending stage advance
  // request. We don't have that joined into fetchProjects, so this is
  // populated separately by the caller via pendingStageProjectIds.
...
// ---------- GLOBAL SEARCH ----------
// Searches across already-loaded data rather than issuing a new query per
// keystroke. Fine at the scale of one FM department's data; if this ever
// needs to scale to thousands of records, switch to a server-side ILIKE
// query or Postgres full-text search instead.
